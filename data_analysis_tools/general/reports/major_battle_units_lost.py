...
class MajorBattleUnitsLost(Report):
    """
    A class that generates reports on the location of major battles based on unit death data.
    ...
    def __init__(
        ...
        """
        Initialize the MajorBattleUnitsLost class with a data retriever, time of battle, and location.
    ...
    def configure_report(self) -> list:
        """
        Configure the report by retrieving unit death data based on the time and location of the battle.

        :return: A list of unit data retrieved based on the battle's location and time.
        """
        return self.data_retriever.get_units_lost_by_location_and_time()
