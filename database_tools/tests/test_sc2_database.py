...
def test_add_plays(setup_database):
    ...
    SC2ReplayDB.add_plays(plays)
    # Retrieve all plays from the database
    all_plays = SC2ReplayDB.get_all_plays()
...
def test_add_players(setup_database):
    ...
    SC2ReplayDB.add_players(players)
    # Retrieve all players from the database
    all_players = SC2ReplayDB.get_all_players()
...
def test_add_games(setup_database):
    ...
    SC2ReplayDB.add_games(games)
    # Retrieve all games from the database
    all_games = SC2ReplayDB.get_all_games()
...
def test_get_player_by_name(setup_database):
    ...
    SC2ReplayDB.add_players([(player_id, player_name)])
    # Retrieve the player by name
    retrieved_player = SC2ReplayDB.get_player_by_name(player_name)
...
def test_get_player_by_id(setup_database):
    ...
    retrieved_player = SC2ReplayDB.get_player_by_id(player_id)
    # Check if the player was retrieved correctly
    assert retrieved_player == (player_id, player_name)
...
def test_get_players_in_game(setup_database):
    ...
    players_in_game = SC2ReplayDB.get_players_in_game(1)
...
def test_get_play(setup_database):
    ...
    SC2ReplayDB.add_plays(plays)
    # Retrieve a play
    race, winner, commands = SC2ReplayDB.get_play(1, 1)
